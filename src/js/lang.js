...
//   },
//   about_title: {
//     ua: 'Я професіонал який вміє знаходити ідеальну локалізацію',
//     ru: 'Я профессионал умеющий находить идеальную локализацию',
//     pl: 'Jestem profesjonalistą, który wie jak znaleźć idealną lokalizację',
//     en: 'I am a professional who knows how to find the perfect localization',
//   },
//   about_description: {
...
const htmlEl = document.querySelector('html'); // Використовуємо тут
const langButtons = document.querySelectorAll('.btn');
const allLangs = ['ua', 'ru', 'pl', 'en'];
const currentPathName = window.location.pathname;

let currentLang = 'ua'; // Початкове значення
let currentTexts = {};
...
const homeTexts = {
  ...
  // ... (весь об'єкт homeTexts залишається без змін)
  'service_btn-3-2': {
    ua: 'Подзвонити',
    ru: 'Позвонить',
    pl: 'Dzwonić',
  ...
  form_answer22: {
    ua: 'Зв`яжуся з Вами протягом однієї години',
    ru: 'Я свяжусь с Вами в течении одного часа',
    pl: 'Skontaktuję się z Tobą w ciągu однієї години',
    en: 'I will contact You within one hour',
  },
  // ... (весь об'єкт homeTexts залишається без змін)
};
...
/**
 * Змінює мову тексту на сторінці та встановлює атрибут lang.
 * @param {string} langCode Код мови ('ua', 'ru', 'pl', 'en').
 */
function updateLanguage(langCode) {
  // 1. Встановлення нового коду мови
  currentLang = langCode;

  // 2. Зміна атрибута lang на <html>
  htmlEl.setAttribute('lang', currentLang);

  // 3. Зміна тексту на сторінці
  for (const key in currentTexts) {
    let elem = document.querySelector(`[data-lang=${key}]`);
    if (elem) {
      // Перевіряємо, чи існує переклад для поточної мови, інакше використовуємо код мови
      elem.textContent = currentTexts[key][currentLang] || currentLang;
    }
  }
}
...
// Перевірка мови браузера (залишена для ініціалізації)
function checkBrowserLang() {
  const navLang = navigator.language.slice(0, 2).toLowerCase();
  const result = allLangs.some(elem => {
    return elem === navLang;
...
// Ініціалізація:
// 1. Перевіряємо, чи є мова в localStorage (якщо функціонал буде додано)
// 2. Перевіряємо мову браузера
// 3. За замовчуванням 'ua'
let initialLang =
  localStorage.getItem('language') || checkBrowserLang() || 'ua';
currentLang = initialLang;

// Встановлення початкової мови та активного класу
function initializeLanguage() {
  updateLanguage(currentLang);
  const activeBtn = document.querySelector(`[data-btn="${currentLang}"]`);
  if (activeBtn) {
    resetActiveClass(langButtons, 'header__btn_active');
    activeBtn.classList.add('header__btn_active');
  }
}
initializeLanguage();

// Вешаємо обробники на кожну кнопку
langButtons.forEach(btn => {
  btn.addEventListener('click', event => {
    const newLang = event.currentTarget.dataset.btn;

    // Перевіряємо, чи не є нова мова поточною
    if (newLang !== currentLang) {
      // 1. Зміна мови тексту та атрибута lang
      updateLanguage(newLang);
      // localStorage.setItem('language', newLang); // Розкоментувати, якщо потрібно зберігати вибір

      // 2. Зміна активного класу
      resetActiveClass(langButtons, 'header__btn_active');
      event.currentTarget.classList.add('header__btn_active');
    }
  });
});
